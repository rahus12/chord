...
# create addresses
address_list = list(map(lambda port: Address('127.0.0.1', port), ports_list))
# keep unique ones
address_list = sorted(set(address_list))

# hash the addresses
hash_list = list(map(lambda addr: hash(addr), address_list))
# create the nodes
locals_list = []
...
while True:
    ...
    if command == "add_node":
        while True:
            ...
            if hash(address) not in hash_list:
